...
export function Scan(bus) {
	...
	const addys = [0x58, 0x59, 0x5A, 0x5B, 0x5C, 0x5D, 0x5E, 0x5F];
	const found = [];
	...
	for (const addr of addys) {
		if (bus.IsSystemBus()) {
			...
			if (result === 0){
				const vendor = bus.ReadByte(addr, 0x43);

				if (vendor === 0x1C){
					const model = bus.ReadByte(addr, 0x44);
...
function WritePacketV4() {
	const vLedPacket = [0x0A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];

	// Set Colors.
	for (var iIdx = 0; iIdx < ciLedCount; iIdx++){
		const iRedIdx = iIdx * 3 + 1;
		const iGrnIdx = iIdx * 3 + 2;
		const iBlueIdx = iIdx * 3 + 3;

		var vColor;

		if (LightingMode === "Forced") { vColor = hexToRgb(forcedColor); } else { vColor = device.color(vLedPositions[iIdx][0], vLedPositions[iIdx][1]); }
