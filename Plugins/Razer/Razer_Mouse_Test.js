...
export function Initialize() {
	...
	if(OnboardDPI) {
		...
		Razer.setDeviceMode("SoftwareMode");
	}
...
function grabColors(shutdown = false) {
	...
	Razer.setMouseLighting(RGBData);

	if(Razer.Config.SupportedFeatures.Hyperflux) {
		const PadRGBData = [];
		const hyperflux = Razer.LEDLibrary["Hyperflux Pad"];

		for(let iIdx = 0; iIdx < 12; iIdx++){
		...
		Razer.setMouseLighting(PadRGBData, 12); //MMM Hardcoding.
	}
}
...
class RazerProtocol {
	...
	setDeviceLightingProperties() {
		const layout = this.LEDLibrary[this.PIDLibrary[device.productId()]];
		vLedNames = [];
		vLedPositions = [];

		if(layout) {
			device.log("Valid Library Config found.");
			device.setName("Razer " + this.PIDLibrary[device.productId()]);
		...
		if(layout.hyperflux) {
			...
			const hyperflux = this.LEDLibrary["Hyperflux Pad"];
...
const DPIConfig =
{
	...
	callback: function(dpi) { return Razer.setDeviceSoftwareDPI(dpi); }
};
