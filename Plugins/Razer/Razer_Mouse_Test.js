...
export class RazerProtocol {
	...
	detectSupportedFeatures() { //This list is not comprehensive, but is a good start.
		...
		const IdleTimeoutSupport = RazerMouse.getDeviceIdleTimeout();

		if(IdleTimeoutSupport !== -1) {
			this.Config.SupportedFeatures.IdleTimeoutSupport = true;
		}

		RazerMouse.determineMouseType();
	}
	...
	getDeviceChargingStatus() {
		...
		if(returnPacket[0] !== 2) {
			...
			device.log("Error fetching Device Battery Level. Error Code: " + this.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	getDeviceBatteryLevel() {
		...
		if(returnPacket[0] !== 2 || returnPacket[9] === undefined) {
	...
	getDeviceSerial() {
		...
		if(returnPacket[0] !== 2) {
			device.log("Error fetching Device Serial. Error Code: " + this.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	getDeviceFirmwareVersion() {
		...
		if(returnPacket[0] !== 2) {
			device.log("Error fetching Device Firmware Version. Error Code: " + this.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	getDeviceLEDZones() {
		...
		const legacyProtocol = RazerMouse.getLegacyMouseLEDBrightness(0);
		...
		for(let zones = 0; zones < 30; zones++) {
			RazerMouse.setModernMouseLEDBrightness(100);
	...
	getDevicePollingRate() {
		let pollingRate;
		Razer.ConfigPacketSend([ 0x01, 0x00, 0x85 ]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error fetching Current Device Polling Rate. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
		...
		const secondaryreturnPacket = Razer.ConfigPacketSend([0x01, 0x00, 0xC0]);
	...
	getCurrentlyConnectedDongles() { //Also of note: return[0] gives 2, and return[4] gives 1 on Blackwidow. Dualpaired Naga.
		...
		if(returnPacket[0] !== 2 || returnPacket[10] === undefined || returnPacket[11] === undefined || returnPacket[13] === undefined || returnPacket[14] === undefined) { //This entire function needs a lot of love.
...
class RazerMouseFunctions {
	...
	determineMouseType() {
		const LegacyMouse = RazerMouse.getLegacyMouseLEDBrightness(0);
	...
	setDeviceLOD(asymmetricLOD, liftOffDistance) {
		Razer.ConfigPacketSend([0x04, 0x0b, 0x0b, 0x00, 0x04, (asymmetricLOD ? 0x02 : 0x01), (liftOffDistance - 1)]);
	}
	/** Function to fetch a device's onboard DPI levels. We do not currently parse this at all.*/
	getDeviceCurrentDPI() {
		Razer.ConfigPacketSend([ 0x07, 0x04, 0x85, 0x00 ]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2 || returnPacket[9] === undefined || returnPacket[10] === undefined || returnPacket[11] === undefined || returnPacket[12] === undefined) {
			device.log("Error fetching Current Device DPI. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	setDeviceSoftwareDPI(dpi) {
		Razer.ConfigPacketSend([0x07, 0x04, 0x05, 0x00, Math.floor(dpi/256), dpi%256, Math.floor(dpi/256), dpi%256]);
		device.pause(20);
	}
	/** Function to fetch a device's onboard DPI levels.*/
	getDeviceDPIStages() //DPI6 does not get included in here.
	{
		Razer.ConfigPacketSend([ 0x26, 0x04, 0x86, 0x00 ]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error fetching Current Device Mode. Error Code: " + Razer.DeviceResponses[returnPacket[0]]);
	...
	setDeviceDPI(stage) {
		...
		Razer.ConfigPacketSend(packet);
		device.pause(50);

	}
	/** Function to fetch the scroll mode from supported mice. */
	getDeviceScrollMode() {
		Razer.ConfigPacketSend([0x02, 0x02, 0x94]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error fetching Current Device Scroll Mode. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	setDeviceScrollMode(ScrollMode) {
		Razer.ConfigPacketSend([0x02, 0x02, 0x14, 0x01, (ScrollMode ? 0x01 : 0x00)]);
	}
	/** Function to fetch the Scroll Acceleration mode from supported mice. */
	getDeviceScrollAccel() {
		Razer.ConfigPacketSend([0x02, 0x02, 0x96]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error fetching Current Device Scroll Acceleration Setting. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	setDeviceScrollAccel(ScrollAccel) {
		Razer.ConfigPacketSend([0x02, 0x02, 0x16, 0x01, (ScrollAccel ? 0x01 : 0x00)]);
	}
	/** Function to fetch the SmartReel Status of a supported mouse */
	getDeviceSmartReel() {
		Razer.ConfigPacketSend([0x02, 0x02, 0x97]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error fetching Current Device Smart Reel Setting. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	setDeviceSmartReel(SmartReel) {
		Razer.ConfigPacketSend([0x02, 0x02, 0x17, 0x01, (SmartReel ? 0x01 : 0x00)]);
	}
	/** Function to fetch the device idle timeout on supported devices. */
	getDeviceIdleTimeout() {
		Razer.ConfigPacketSend([0x02, 0x07, 0x83]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error fetching Current Device Idle Timeout Setting. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	setMouseLighting(RGBData, NumberOfLEDs  = Razer.Config.NumberOfLEDs, hyperflux = false) {
		if(this.Config.MouseType === "Modern") {
			this.setModernMouseDeviceColor(RGBData, NumberOfLEDs, hyperflux);
		} else {
	...
	setLegacyMouseLEDColor(zone, rgbdata) {//Color for Deathadder Chroma
		...
		Razer.StandardPacketSend([ 0x05, 0x03, 0x01, 0x00, zone, rgbdata[0], rgbdata[1], rgbdata[2] ]);
		Razer.setLegacyLEDEffect(zone);
	}
	/** Function to set a modern mouse's led colors.*/
	setModernMouseDeviceColor(RGBData, NumberOfLEDs, hyperflux) {
		const packet = [(NumberOfLEDs*3 + 5), 0x0F, 0x03, hyperflux, 0x00, 0x00, 0x00, Razer.Config.NumberOfLEDs-1];
		packet.push(...RGBData);
	...
	getLegacyMouseLEDBrightness(led = 0, detection=false) {
		Razer.ConfigPacketSend([ 0x03, 0x03, 0x83, 0x00, led]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			if(!detection) {
				device.log("Error fetching Legacy Device LED Brightness. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
			}
	...
	setLegacyMouseLEDBrightness(brightness, led = 0) {
		Razer.ConfigPacketSend([ 0x03, 0x03, 0x03, 0x00, led, brightness*255/100]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error setting Legacy Device LED Brightness. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
	...
	getModernMouseLEDBrightness(led = 0, detection=false) {
		Razer.ConfigPacketSend([0x03, 0x0f, 0x84, 0x00, led]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			if(!detection) {
				device.log("Error fetching Modern Device LED Brightness. Error Code: " + Razer.DeviceResponses[returnPacket[0]], {toFile : true});
			}
	...
	setModernMouseLEDBrightness(brightness, led = 0) {
		Razer.ConfigPacketSend([0x03, 0x0f, 0x04, 0x01, led, brightness*255/100]);

		const returnPacket = Razer.ConfigPacketRead();

		if(returnPacket[0] !== 2) {
			device.log("Error setting Modern Device LED Brightness. Error Code: " + Razer.DeviceResponses[returnPacket[0]]);
