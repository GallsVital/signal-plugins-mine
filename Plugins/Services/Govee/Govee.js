...
class GoveeController{
	...
	update(){
		if(!this.initialized && (this.supportDreamView || this.supportRazer)){
...
class GoveeProtocol {
	...
	SendRGB(RGBData) {
		if (!this.supportDreamView && !this.supportRazer) {
			device.log("How did we get here? You're not supposed to be detected!");
			//device.pause(10000); // We can't dead lock this thread for 10seconds at a time
		...
		if (this.supportDreamView) {
			...
			packet = this.createRazerPacket(RGBData);
		}
