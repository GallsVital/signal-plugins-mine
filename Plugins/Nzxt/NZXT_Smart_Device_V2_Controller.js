export function Name() { return "NZXT Smart Device V2"; }
export function VendorId() { return 0x1E71; }
export function ProductId() { return Object.keys(NZXTSmartDevice2ProductNames);}
...
export function Initialize() {
	const DeviceName = NZXTSmartDevice2ProductNames[device.productId()] || "NZXT Smart Device V2";
...
function SendChannel(Channel, shutdown = false) {
	...
	while(ChannelLedCount > 0){
		...
		SmartDevice2.StreamLightingPacketChanneled(packetNumber, ledsToSend, RGBData.splice(0, ledsToSend * 3), Channel);
...
function HandleFanPacket(data){
	...
	for(let fanId = 0; fanId < 3;fanId++){
		const rpm = data[SmartDevice2.offsetFanRPM + fanId * 2] | (data[SmartDevice2.offsetFanRPM + fanId * 2 + 1] << 8);
		const mode = SmartDevice2.FanModes[data[SmartDevice2.offsetFanMode + fanId]];
...
class NZXTSmartDevice2{
	...
	SetChannelCount(count){
		...
		device.SetLedLimit(Math.min(this.MaxDeviceLeds, this.MaxChannelLeds * count));
	}
